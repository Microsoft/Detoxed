...
class IntegTestBase(ABC):
    ...
    def test(self) -> TestResult:
        """Run tests against the functionality under test."""
...
class IntegTestRunner:
    ...
    def run(self) -> bool:
        """Run integration test suite, obeying the deployment condition."""
        if not self.tests:
    ...
    def _block_on_deployment_condition(self):
        """Block on deployment condition or fail with a TimeoutError if the condition is not met within the timeout."""
        start = time()
        while True:
